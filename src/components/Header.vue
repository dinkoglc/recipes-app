<template>
  <!-- <v-app-bar color="grey lighten-3">
    <v-toolbar-title style="color: white">Recipes App</v-toolbar-title> <v-spacer></v-spacer>
    <v-btn text to="/" style="color: white">Home</v-btn>
    <v-btn text to="/login" style="color: white">Login</v-btn>
  </v-app-bar> -->

  <v-toolbar style="background-color: rgba(255, 215, 0, 0.2)">
    <v-toolbar-title style="color: black">Recipes App</v-toolbar-title>
    <v-spacer></v-spacer>
    <v-btn text to="/landing" style="color: black">Home</v-btn>
    <v-btn text to="/login" style="color: black">Login</v-btn>
  </v-toolbar>
</template>
<script>
export default {}
</script>
<style scoped></style>
